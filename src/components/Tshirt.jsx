...
function Tshirt() {
  ...
  const [fabricRange, setFabricRange] = useState(null);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:5000/Tshirt");
        if (!response.ok) {
        ...
        const validatedProducts = data.map((product) => ({
          ...product,
          price: parseFloat(product.originalPrice) || 0,
          ...
          // Fix image handling - ensure proper image URL
          image:
            product.image ||
  ...
  const handleFabricChange = (fabric) => setFabricRange(fabric);
  ...
  // Fixed navigation path for t-shirts
  const navigateToDetails = (id, brand) => {
    navigate(`/tshirt/${brand.toLowerCase()}/${id}`);
  };
  ...
  let filteredProducts = products.filter((product) => {
    ...
    const matchesFabric = !fabricRange || product.fabric === fabricRange;
        ...
        <div className="container">
          <div className="row mb-4">
            <div className="col-md-6">
              <h1 className="mb-0 text-primary fw-bold">Premium T-Shirts</h1>
              <p className="text-muted">Wear your style with comfort</p>
            </div>
            <div className="col-md-6">
                ...
                <div className="sticky-top" style={{ top: "20px" }}>
                  ...
                  <FilterSection title="Fabric">
                    {uniqueFabrics.map((fabric) => (
                      <div className="form-check" key={fabric}>
                        <input
                          className="form-check-input"
                          type="radio"
                          name="fabric"
                          checked={fabricRange === fabric}
